...
def editar_armario(request, id):
    ...
    form_armario = armario_forms.ArmarioForm(request.POST or None, instance=armario_antigo)   
    if form_armario.is_valid():
        if form_armario.is_valid():
            usuario = form_armario.cleaned_data["usuario"]
            nome = form_armario.cleaned_data["nome"]
            ...
            armario_novo = armario.Armario(usuario=request.user, nome=nome, sigla=sigla, tombo=tombo, sala=sala, obs=obs, 
            fotoarmario=fotoarmario)              
