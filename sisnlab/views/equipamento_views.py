...
def inserir_equipamento(request):
    if request.method == "POST":
        ...
        if form_equipamento.is_valid():
            ...
            equipamento_novo = equipamento.Equipamento(nome=nome, tombo=tombo, marca=marca, data_compra=data_compra, data_ultim_m=data_ultim_m, origem=origem, ficha_tec=ficha_tec, especficacao_t=especficacao_t, calibragem=calibragem, sala=sala, armario=armario, bancada=bancada, estante=estante, prateleira=prateleira, gaveta=gaveta, obs=obs)
...
def editar_equipamento(request, id):
    ...
    if form_equipamento.is_valid():
        if form_equipamento.is_valid():
            ...
            equipamento_novo = equipamento.Equipamento(nome=nome, tombo=tombo, marca=marca, data_compra=data_compra, 
            data_ultim_m=data_ultim_m, origem=origem, ficha_tec=ficha_tec, especficacao_t=especficacao_t, calibragem=calibragem, 
            sala=sala, armario=armario, bancada=bancada, estante=estante, prateleira=prateleira, gaveta=gaveta, obs=obs)
