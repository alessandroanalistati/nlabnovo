...
def inserir_vidraria(request):
    if request.method == "POST":
        ...
        if form_vidraria.is_valid():
            ...
            vidraria_novo = vidraria.Vidraria(nome=nome, marca=marca, data_compra=data_compra, origem=origem, ficha_tec=ficha_tec, especficacao_t=especficacao_t, 
            quantidade=quantidade, sala=sala, armario=armario, bancada=bancada, estante=estante, prateleira=prateleira, gaveta=gaveta, obs=obs)
...
def editar_vidraria(request, id):
    ...
    form_vidraria = vidraria_forms.VidrariaForm(request.POST or None, instance=vidraria_antigo)   
    if form_vidraria.is_valid():
            nome = form_vidraria.cleaned_data["nome"]                   
            marca = form_vidraria.cleaned_data["marca"]            
            data_compra = form_vidraria.cleaned_data["data_compra"]            
